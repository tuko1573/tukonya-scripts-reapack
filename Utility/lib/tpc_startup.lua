--[[
  tpc_startup.lua
  Team Plugin Checker — REAPER起動時の自動送信を仕込む／外す。
...
M.BEGIN_MARK = "-- BEGIN TeamPluginChecker (managed; safe to delete)"
M.END_MARK = "-- END TeamPluginChecker"

-- 旧版（Shippo Blend Plugins）が書いた一塊の印。install のときに見つけたら消す（二重送信を防ぐ）。
M.LEGACY_BEGIN_MARK = "-- BEGIN ShippoBlend (managed; safe to delete)"
M.LEGACY_END_MARK = "-- END ShippoBlend"
...
function M.block(subpath)
  ...
  rel = rel .. "TUKONYA_Team Plugin Checker (Send Now).lua"
...
--- 旧版の一塊（LEGACY_BEGIN_MARK〜LEGACY_END_MARK、END直後の改行1つまで）を取り除く。
-- 印の外側は1バイトも変えない（前後の空行も残す）。複数あれば全部消す。
-- @return new_content, removed(bool)
function M.remove_legacy_block(content)
  content = content or ""
  local removed = false
  while true do
    local s, after = find_block(content, M.LEGACY_BEGIN_MARK, M.LEGACY_END_MARK)
    if not s then break end
    content = content:sub(1, s - 1) .. content:sub(after)
    removed = true
  end
...
function M.install(deps)
  ...
  local path = M.startup_path(deps)
  local content = M.remove_legacy_block(read_startup(deps))
